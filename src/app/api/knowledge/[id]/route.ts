...
export async function PATCH(
  ...
  const { name, category, placeId } = body as {
    name?: string;
    category?: string;
    // null heisst ausdruecklich "kein Ort", undefined heisst "nicht angefasst".
    placeId?: number | null;
  };
  ...
  const target = await db
    ...
    .where(
      and(
        eq(productKnowledge.id, Number(id)),
  ...
  // Der gelernte Ort ist die zweite Haelfte derselben Aussage: Kategorie und
  // Fach beantworten beide die Frage "wohin gehoert das hier?", und beide
  // kommen beim naechsten Erfassen zurueck. Korrigierbar war bisher nur die
