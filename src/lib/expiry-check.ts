...
/**
 * Ein Durchlauf der Ablauf-Erinnerungen ueber alle Listen.
 *
 * Steht als Bibliothek und nicht mehr nur als Route da, weil die App den Lauf
 * seit dem eingebauten Zeitgeber selbst anstoesst -- ein HTTP-Aufruf an die
 * eigene Adresse waere dafuer ein Umweg ueber das Netzwerk, der nur
 * fehlschlagen kann.
 *
 * `respectPreferredHour` haelt sich an die pro Nutzer eingestellte Uhrzeit.
 * Das setzt einen stuendlichen Lauf voraus: wer den Job weiterhin einmal am
 * Tag von aussen anstoesst, laesst den Schalter aus und bekommt wie bisher
 * bei jedem Lauf alles Faellige -- eine stillschweigende Zeitpruefung wuerde
 * dort schlicht nie zutreffen und die Erinnerungen fuer immer verstummen
 * lassen.
 */
